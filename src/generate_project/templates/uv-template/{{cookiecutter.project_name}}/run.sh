...
function run {
    ...
    if [ -f "$THIS_DIR/src/{{ cookiecutter.package_name }}/main.py" ]; then
    ...
    echo "  uv run uvicorn {{ cookiecutter.package_name }}.server:app --reload"
    echo ""
...
function tests:cov {
    ...
    TEST_FILE="${1:-$(get:python:files:tests)}"
    shift || true
    uv run pytest "$TEST_FILE" --cov=src/{{ cookiecutter.package_name }} --cov-report=term "$@"
}
...
function docs:api {
    ...
    cd docs && uv run sphinx-apidoc -o api ../src/{{ cookiecutter.package_name }} -f
}
