...
function run {
    ...
    if [ -f "$THIS_DIR/examples/main.py" ]; then
        poetry run python "$THIS_DIR/examples/main.py" "$@"
        return
    fi
    ...
    echo "  poetry run uvicorn {{ cookiecutter.package_name }}.server:app --reload"
    echo ""
...
function tests:cov {
    ...
    TEST_FILE="${1:-$(get:python:files:tests)}"
    shift || true
    poetry run pytest "$TEST_FILE" --cov=src/{{ cookiecutter.package_name }} --cov-report=term "$@"
}
